...
contains
  ...
  subroutine ice_ocean_flux_exchange_init(Time, Ice, Ocean, Ocean_state, Wav, ice_ocean_boundary, &
    ...
    if (Wav%waves_is_init) then
	  ...
	  !-----shuoli202203----------------------------------------
	  allocate( ice_ocean_boundary%hs(is:ie,js:je) ) ; ice_ocean_boundary%hs(:,:) = 0.0
	  allocate( ice_ocean_boundary%k(is:ie,js:je) ) ; ice_ocean_boundary%k(:,:) = 0.0
	  allocate( ice_ocean_boundary%omiga(is:ie,js:je) ) ; ice_ocean_boundary%omiga(:,:) = 0.0
	  !--------------------------------------------------------
    endif
  ...
  subroutine flux_ice_to_ocean ( Time, Ice, Ocean, Ice_Ocean_Boundary, ice_wave_boundary )
	...
	!---------SHUOLI202203---------------------------------------------
	if(ASSOCIATED(ice_wave_boundary%icegrd_hs_mpp) ) call flux_ice_to_ocean_redistribute( Ice, Ocean, &
         ice_wave_boundary%icegrd_hs_mpp(:,:,1), Ice_Ocean_Boundary%hs(:,:), Ice_Ocean_Boundary%xtype, do_area_weighted_flux )
	if(ASSOCIATED(ice_wave_boundary%icegrd_k_mpp) ) call flux_ice_to_ocean_redistribute( Ice, Ocean, &
         ice_wave_boundary%icegrd_k_mpp(:,:,1), Ice_Ocean_Boundary%k(:,:), Ice_Ocean_Boundary%xtype, do_area_weighted_flux )
	if(ASSOCIATED(ice_wave_boundary%icegrd_omiga_mpp) ) call flux_ice_to_ocean_redistribute( Ice, Ocean, &
         ice_wave_boundary%icegrd_omiga_mpp(:,:,1), Ice_Ocean_Boundary%omiga(:,:), Ice_Ocean_Boundary%xtype, do_area_weighted_flux )
	!-------------------------------------------------------------------
