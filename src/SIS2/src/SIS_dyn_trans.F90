...
subroutine SIS_dynamics_trans(IST, OSS, FIA, IOF, dt_slow, CS, icebergs_CS, G, IG, tracer_CSp)
  ...
  do nac=1,nadv_cycle
    ...
    if (CS%merged_cont) then
      ...
      call SIS_merged_dyn_cont(IST, OSS, FIA, IOF, CS%DS2d, dt_adv_cycle, Time_cycle_start, G, IG, CS)!-shuoli-
    ...
    !        call hchksum_pair("WindStr_[xy]_A before SIS_C_dynamics", WindStr_x_A, WindStr_y_A, G, halos=1)
          ...
          if (CS%Warsaw_sum_order) then
            call SIS_C_dynamics(1.0-ice_free(:,:), misp_sum, mi_sum, IST%u_ice_C, IST%v_ice_C, &
                                ...
                                str_x_ice_ocn_Cu, str_y_ice_ocn_Cv, dt_slow_dyn, G, CS%SIS_C_dyn_CSp, IST)!---shuoli---
          else
          ...
          !---------shuoli202111-----------------------------------------------
          !str_x_ice_ocn_Cu = str_x_ice_ocn_Cu + IST%w2i_phice * cos((270-IST%w2i_pkdir)/180*3.14159265359)
          !str_y_ice_ocn_Cv = str_y_ice_ocn_Cv + IST%w2i_phice * sin((270-IST%w2i_pkdir)/180*3.14159265359)
          !------------------------------------------------------------------
...
subroutine SIS_multi_dyn_trans(IST, OSS, FIA, IOF, dt_slow, CS, icebergs_CS, G, IG, tracer_CSp, &
  ...
  do nac=1,nadv_cycle
    ...
    call SIS_merged_dyn_cont(IST, OSS, FIA, IOF, CS%DS2d, dt_adv_cycle, Time_cycle_start, G, IG, CS, &
                             end_call=end_of_cycle) !--SHUOLI202111-----------
...
subroutine SIS_merged_dyn_cont(IST, OSS, FIA, IOF, DS2d, dt_cycle, Time_start, G, IG, CS, end_call) !-SHUOLI202111-
  type(ocean_sfc_state_type), intent(in)    :: OSS !< A structure containing the arrays that describe
                                                   !! the ocean's surface state for the ice model.
