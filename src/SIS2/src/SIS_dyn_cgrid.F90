...
type, public :: SIS_C_dyn_CS ; private
  ...
  real :: Rho_ice = 917.0     !< The nominal density of sea ice [kg m-3].shuoli-905-917
...
#include "version_variable.h"
  ...
  call get_param(param_file, mdl, "RHO_ICE", CS%Rho_ice, &
                 ...
                 units="kg m-3", default=917.0)!-shuoli-905-917
...
subroutine SIS_C_dynamics(ci, mis, mice, ui, vi, uo, vo, &
                          fxat, fyat, sea_lev, fxoc, fyoc, dt_slow, G, CS, IST)
...
!$OMP                                  m_uio_explicit,uio_pred,uio_C)
    do j=jsc,jec ; do I=isc-1,iec
      ...
      ! the ice-ocean drag.--shuoli--try explicit wave stress
        drag_u = 0.0
        if (G%mask2dCu(I,j) > 0.0) then
          m_uio_explicit = uio_init*mi_u(I,j) + dt * &
               ((Cor + PFu(I,j))*mi_u(I,j) + (fxic_now + fxat(I,j) + IST%fxw2i_str(I,j)))
      ...
      uio_C =  G%mask2dCu(I,j) * ( mi_u(I,j) * &
                 ((ui(I,j) + dt * Cor) * I1_f2dt2_u(I,j) - uo(I,j)) + &
                  dt * (mi_u(I,j) * PFu(I,j) + (fxic_now + fxat(I,j) + IST%fxw2i_str(I,j))) ) / &
                 (mi_u(I,j) + m_neglect + dt * drag_u)

      ui(I,j) = (uio_C + uo(I,j)) * G%mask2dCu(I,j)
      ! Note that fxoc is the stress felt by the ocean.
      fxoc(I,j) = fxoc(I,j) + drag_u*uio_C !+ IST%fxw2i_str(I,j) !-SHUOLI---
    ...
    do J=jsc-1,jec ; do i=isc,iec
      ...
      ! the ice-ocean drag.--shuoli--try explicit wave stress
        ...
        if (G%mask2dCv(i,J) > 0.0) then
          m_vio_explicit = vio_init*mi_v(i,J) + dt * &
               ((Cor + PFv(i,J))*mi_v(i,J) + (fyic_now + fyat(i,J) + IST%fyw2i_str(i,J)))
      ...
      vio_C =  G%mask2dCv(i,J) * ( mi_v(i,J) * &
                 ((vi(i,J) + dt * Cor) * I1_f2dt2_v(i,J) - vo(i,J)) + &
                  dt * (mi_v(i,J) * PFv(i,J) + (fyic_now + fyat(i,J) + IST%fyw2i_str(i,J))) ) / &
                 (mi_v(i,J) + m_neglect + dt * drag_v)

      vi(i,J) = (vio_C + vo(i,J)) * G%mask2dCv(i,J)
      ! Note that fyoc is the stress felt by the ocean.
      fyoc(i,J) = fyoc(i,J) + drag_v*vio_C !+ IST%fyw2i_str(i,J) !-shuoli-----
...
!$OMP do
  do j=jsc,jec ; do I=isc-1,iec
    fxoc(I,j) = fxoc(I,j) * (G%mask2dCu(I,j) * I_sub_steps) !!+ IST%fxw2i_str(I,j) * G%mask2dCu(I,j)
  ...
  !-----------------shuoli-----------
  !do j=jsc,jec ; do i=isc,iec
  !fxoc(i,j) = (fxoc(i,j) + IST%fxw2i_str(i,j)) * G%mask2dCu(i,j)
  !enddo ; enddo
  !do j=jsc,jec
  !fxoc(isc-1,j) = (2 * fxoc(isc,j) - fxoc(isc+1,j)) * G%mask2dCu(isc-1,j)
  !enddo
  !----------------------------------
!$OMP end do nowait
!$OMP do
  do J=jsc-1,jec ; do i=isc,iec
    fyoc(i,J) = fyoc(i,J) * (G%mask2dCv(i,J) * I_sub_steps) !!+ IST%fyw2i_str(I,j) * G%mask2dCv(I,j)
  ...
  !-----------------shuoli-----------
  !do j=jsc,jec ; do i=isc,iec
  !fyoc(i,j) = (fyoc(i,j) + IST%fyw2i_str(i,j)) * G%mask2dCv(i,j)
  !enddo ; enddo
  !do i=isc,iec
  !fyoc(i,jsc-1) = (2 * fyoc(i,jsc) - fyoc(i,jsc+1)) * G%mask2dCv(i,jsc-1)
  !enddo
  !----------------------------------
!$OMP end parallel
        ...
        if ((ui(I,j) < ui_min_trunc(I,j)) .or. (ui(I,j) > ui_max_trunc(I,j))) then
          if (mi_u(I,j) > m_neglect) then
            ...
            call write_u_trunc(I, j, ui, u_IC, uo, mis, fxoc, fxic, Cor_u, &
                               PFu, fxat, dt_slow, G, CS, IST%fxw2i_str)
          endif
          if (ui(I,j) < ui_min_trunc(I,j)) then
        ...
        if ((vi(i,J) < vi_min_trunc(i,J)) .or. (vi(i,J) > vi_max_trunc(i,J))) then
          if (mi_v(i,J) > m_neglect) then
            ...
            call write_v_trunc(i, J, vi, v_IC, vo, mis, fyoc, fyic, Cor_v, &
                               PFv, fyat, dt_slow, G, CS, IST%fyw2i_str)
          endif
          if (vi(i,J) < vi_min_trunc(i,J)) then
...
subroutine write_u_trunc(I, j, ui, u_IC, uo, mis, fxoc, fxic, Cor_u, PFu, fxat, &
  ...
  real, dimension(SZIB_(G),SZJ_(G)), intent(in) :: fxw2istr !< The zonal wave to ice stress [Pa].
  ...
  ! Open up the file for output if this is the first call.
    ...
    write (file, '("U change due to fxat, fxwaveice, fxoc, fxic, Cor_u, PFu = ", 5ES11.3, " sum = ",ES11.3)') &
      fxat(I,j)*dt_mi, fxw2istr(I,j)*dt_mi, -fxoc(I,j)*dt_mi, fxic(I,j)*dt_mi, Cor_u(I,j)*dt_slow, PFu(I,j)*dt_slow, &
      (fxw2istr(I,j) + fxat(I,j) - fxoc(I,j) + fxic(I,j))*dt_mi + (Cor_u(I,j) + PFu(I,j))*dt_slow
...
subroutine write_v_trunc(i, J, vi, v_IC, vo, mis, fyoc, fyic, Cor_v, PFv, fyat, &
                         dt_slow, G, CS, fyw2istr)
  integer,                           intent(in) :: i    !< The i-index of the column to report on
  integer,                           intent(in) :: J    !< The j-index of the column to report on
  type(SIS_hor_grid_type),           intent(in) :: G    !< The horizontal grid type
  real, dimension(SZI_(G),SZJB_(G)), intent(in) :: vi   !< The meridional ice velicity [m s-1].
  real, dimension(SZI_(G),SZJB_(G)), intent(in) :: v_IC !< The initial meridional ice velicity [m s-1].
  real, dimension(SZI_(G),SZJB_(G)), intent(in) :: vo   !< The meridional ocean velicity [m s-1].
  real, dimension(SZI_(G),SZJ_(G)),  intent(in) :: mis  !< The mass of ice an snow per unit ocean area [kg m-2]
  ...
  real, dimension(SZI_(G),SZJB_(G)), intent(in) :: Cor_v !< The meridional Coriolis acceleration [m s-2].
  ...
  real, dimension(SZI_(G),SZJB_(G)), intent(in) :: fyw2istr !< The meridional wind stress [Pa].
  real,                              intent(in) :: dt_slow !< The slow ice dynamics timestep [s].
  type(SIS_C_dyn_CS),                pointer    :: CS   !< The control structure for this module
  ...
  ! Open up the file for output if this is the first call.
    ...
    write (file, '("V change due to fyat, fywaveice, fyoc, fyic, Cor_v, PFv = ", 5ES11.3, " sum = ",ES11.3)') &
      fyat(i,J)*dt_mi, fyw2istr(i,J)*dt_mi, -fyoc(i,J)*dt_mi, fyic(i,J)*dt_mi, Cor_v(i,J)*dt_slow, PFv(i,J)*dt_slow, &
      (fyw2istr(i,J) + fyat(i,J) - fyoc(i,J) + fyic(i,J))*dt_mi + (Cor_v(i,J) + PFv(i,J))*dt_slow
