...
type ice_data_type !  ice_public_type
  ...
  real, pointer, dimension(:,:) :: &
    ...
    !-------------------shuoli 202111----------------------------
    !These fields are used to provide information about the wave to ice
    !the exchange intoduced in ice_ocean_flux_exchange, together with 
    !ice_wave_boundary passing to ice_oean_b.. and ice passing to ice_ocean_b..
    !real, pointer, dimension(:,:) :: &
    !  wv_phice => NULL() , &    ! wave to ice stress kg/m3
    !  wv_pkdir => NULL()        ! wave to ice peak direction degree
  !-----------------------------------------------------------
