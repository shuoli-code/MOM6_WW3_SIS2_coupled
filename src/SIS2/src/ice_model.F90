...
use wave_type_mod,   only:  ice_wave_boundary_type
...
subroutine update_ice_model_slow(Ice) !---shuoli202111---
  type(ice_data_type), intent(inout) :: Ice !< The publicly visible ice data type.

  !--------------shuoli202111----
  !type(ice_wave_boundary_type), intent(in) :: waves2ice !
  !---
  !type(SIS_hor_grid_type), pointer :: sG => NULL()
  !-------------------------------

  !---shuoli202111----------------
  !sync wave2ice:phice and pkdir to Ice structure
  !ice%wv_phice(:,:) = waves2ice%icegrd_phice_mpp(:,:,1)
  !ice%wv_pkdir(:,:) = waves2ice%icegrd_pkdir_mpp(:,:,1)
  !sG => Ice%sCS%G
  !Ice%sCS%IST%w2i_phice(:,:) = waves2ice%icegrd_phice_mpp(:,:,1)
  !Ice%sCS%IST%w2i_pkdir(:,:) = waves2ice%icegrd_pkdir_mpp(:,:,1)
  !-------------------------------
...
subroutine update_ice_dynamics_trans(Ice, time_step, start_cycle, end_cycle, cycle_length)
  ...
  !-------shuoli202111---------------------------------
  !type(ice_wave_boundary_type), optional, intent(in) :: waves2ice !  
  !----------------------------------------------------
  ...
  if (sIST%valid_IST) then
    ...
    !----shuoli202111-------------------------------------
    !call pass_var(sIST%w2i_phice, sG%Domain)
    !call pass_var(sIST%w2i_pkdir, sG%Domain)
    !----------------------------------------------------
  endif
