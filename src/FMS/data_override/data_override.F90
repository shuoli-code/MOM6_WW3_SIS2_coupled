...
type(domain2D),save :: ocn_domain,atm_domain,lnd_domain,ice_domain,wav_domain
...
real, dimension(:,:), target, allocatable :: lon_local_wav, lat_local_wav
...
real                                      :: min_glo_lon_wav, max_glo_lon_wav
...
logical                                         :: atm_on, ocn_on, lnd_on, ice_on, wav_on
logical                                         :: lndUG_on
...
subroutine data_override_init(Atm_domain_in, Ocean_domain_in, Ice_domain_in, Land_domain_in, Land_domainUG_in,&
  ...
  type (domain2d), intent(in), optional :: Wave_domain_in
...
!  if(module_is_initialized) return
  ...
  if (wav_on) wav_domain = Wave_domain_in
...
!  Initialize override array
 ...
 if ( .NOT. (atm_on .or. ocn_on .or. lnd_on .or. ice_on .or. lndUG_on .or. wav_on)) return
 call fms_io_init
...
! Test if grid_file is already opened
 ...
 if(field_exist(grid_file, "x_T" ) .OR. field_exist(grid_file, "geolon_t" ) ) then
    ...
    if (wav_on .and. .not. allocated(lon_local_wav) ) then
       call mpp_get_compute_domain( wav_domain,is,ie,js,je)
       allocate(lon_local_wav(is:ie,js:je), lat_local_wav(is:ie,js:je))
       call get_grid_version_1(grid_file, 'wav', wav_domain, is, ie, js, je, lon_local_wav, lat_local_wav, &
            min_glo_lon_wav, max_glo_lon_wav )
    endif
   ...
   if (atm_on .and. .not. allocated(lon_local_atm) ) then
    ...
    if (wav_on .and. .not. allocated(lon_local_wav) ) then
       call mpp_get_compute_domain( wav_domain,is,ie,js,je)
       allocate(lon_local_wav(is:ie,js:je), lat_local_wav(is:ie,js:je))
       call get_grid_version_2(grid_file, 'wav', wav_domain, is, ie, js, je, lon_local_wav, lat_local_wav, &
            min_glo_lon_wav, max_glo_lon_wav )
    endif
 else
...
subroutine data_override_unset_domains(unset_Atm, unset_Ocean, &
  ...
  logical, intent(in), optional :: unset_Atm, unset_Ocean, unset_Ice, unset_Land, unset_Wave
...
! </NOTE>
  ...
  if (PRESENT(unset_Wave)) then ; if (unset_Wave) then
    if (fail_if_not_set .and. .not.wav_on) call mpp_error(FATAL, &
      "data_override_unset_domains attempted to work on a Wav_domain that had not been set.")
    wav_domain = NULL_DOMAIN2D
    wav_on = .false.
    if (allocated(lon_local_wav)) deallocate(lon_local_wav)
    if (allocated(lat_local_wav)) deallocate(lat_local_wav)
  endif ; endif
...
! Grid name (Ocean, Ice, Atmosphere, Land, Wave)
!   </IN>
...
! convert lon_in and lat_in from deg to radian
        ...
        case('WAV')
           lon_local => lon_local_wav; lat_local => lat_local_wav
        case default
...
!2 Global lon and lat of ocean grid cell centers are determined from adjacent vertices
  ...
  case('atm', 'lnd', 'wav')
     if(trim(mod_name) == 'atm') then
        xname = 'xta'; yname = 'yta'
     elseif(trim(mod_name) == 'lnd') then
        xname = 'xtl'; yname = 'ytl'
     elseif(trim(mod_name) == 'wav') then
        xname = 'xtw'; yname = 'ytw'
     endif
     ...
     deallocate(glat)
  case default
     call mpp_error(FATAL, "data_override_mod: mod_name should be 'atm', 'ocn', 'ice', 'lnd', or 'wav' ")
  end select
...
subroutine get_grid_version_2(mosaic_file, mod_name, domain, isc, iec, jsc, jec, lon, lat, min_lon, max_lon)
  ...
  if(trim(mod_name) .NE. 'atm' .AND. trim(mod_name) .NE. 'ocn' .AND. &
     trim(mod_name) .NE. 'ice' .AND. trim(mod_name) .NE. 'lnd' .AND. &
     trim(mod_name) .NE. 'wav' ) call mpp_error(FATAL, &
        "data_override_mod: mod_name should be 'atm', 'ocn', 'ice', 'lnd', or 'wav' ")
