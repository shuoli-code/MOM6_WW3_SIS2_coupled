...
type, public :: wave_parameters_CS ; private
  ...
  !-------shuoli202203----------------------------------
  real, allocatable, dimension(:,:), public :: &
       hs_wv,&            !< significant wave height at H points
	   k_wv,&            !< wave number at H points
	   omiga_wv            !< peak angular frequency at H points
  !---------------------------------------------------------
  real, allocatable, dimension(:,:), public :: &
...
subroutine MOM_wave_interface_init(time, G, GV, US, param_file, CS, diag )
  ...
  !--------------shuoli202203----------------------------------
  allocate(CS%hs_wv(G%isd:G%Ied,G%jsd:G%jed))
  CS%hs_wv(:,:) = 0.0
  allocate(CS%k_wv(G%isd:G%Ied,G%jsd:G%jed))
  CS%k_wv(:,:) = 0.0
  allocate(CS%omiga_wv(G%isd:G%Ied,G%jsd:G%jed))
  CS%omiga_wv(:,:) = 0.0
  !------------------------------------------------------------
...
subroutine Update_Surface_Waves(G, GV, US, Day, dt, CS, forces)
  ...
  do ii=G%isc,G%iec
    do jj=G%jsc,G%jec
            CS%hs_wv(ii,jj) = forces%hs_wave(ii,jj)
			CS%k_wv(ii,jj) = forces%k_wave(ii,jj)
			CS%omiga_wv(ii,jj) = forces%omiga_wave(ii,jj)
    enddo
  enddo
  call pass_var(CS%hs_wv, G%Domain)
  call pass_var(CS%k_wv, G%Domain)
  call pass_var(CS%omiga_wv, G%Domain)
  !-------------------------------------------------------
  return
...
subroutine Waves_end(CS)
  ...
  !---------shuoli202203----------------------------------
  if (allocated(CS%hs_wv))       deallocate( CS%hs_wv )
  if (allocated(CS%k_wv))        deallocate( CS%k_wv )
  if (allocated(CS%omiga_wv))    deallocate( CS%omiga_wv )
  !-------------------------------------------------------
  deallocate( CS )
