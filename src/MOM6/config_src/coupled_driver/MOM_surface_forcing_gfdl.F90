...
type, public :: ice_ocean_boundary_type
  ...
  !-------------------shuoli202203-------------------------
  real, pointer, dimension(:,:)   :: hs           => NULL() !<
  real, pointer, dimension(:,:)   :: k           => NULL() !<
  real, pointer, dimension(:,:)   :: omiga           => NULL() !<
  !----------------------------------------------------------
...
subroutine convert_IOB_to_forces(IOB, forces, index_bounds, Time, G, US, CS, dt_forcing, reset_avg)
  ...
  do j=js,je; do i=is,ie
	 ...
	 !--------------shuoli202203-------------------------
	 forces%hs_wave(i,j) = IOB%hs(i-I0,j-J0)
	 forces%k_wave(i,j) = IOB%k(i-I0,j-J0)
	 forces%omiga_wave(i,j) = IOB%omiga(i-I0,j-J0)
	 !--------------------------------------------------
  enddo ; enddo
  call pass_vector(forces%ustk0,forces%vstk0, G%domain )
  !--------------shuoli202203-------------------------
  call pass_var(forces%hs_wave, G%Domain)
  call pass_var(forces%k_wave, G%Domain)
  call pass_var(forces%omiga_wave, G%Domain)
  !--------------------------------------------------
